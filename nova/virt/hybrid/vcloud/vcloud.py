...
from pyvcloud.schema.vcd.v1_5.schemas.vcloud import vAppType, \
    ...
    networkType, vcloudType, taskType, vAppTemplateType, mediaType
...
class VCA(sdk_vca):
    ...
    def cancel_upload(self, task_cancel_href):
        response = self._invoke_api(requests, 'post',
                                                task_cancel_href,
                                                headers=self.vcloud_session.get_vcloud_headers(),
                                                verify=self.verify)
    ...
    def get_upload_media_task(self, catalog_name, media_name):
        refs = filter(lambda ref: ref.name == catalog_name and ref.type_ == 'application/vnd.vmware.vcloud.catalog+xml', self.vcloud_session.organization.Link)
        if len(refs) == 1:
            response = self._invoke_api(requests, 'get',
                                        refs[0].get_href(),
            ...
            if response.status_code == requests.codes.ok:
                ...
                if len(catalog_items) == 1:
                    ...
                    if response.status_code == requests.codes.ok:
                        ...
                        response = self._invoke_api(requests, 'get', href, headers=self.vcloud_session.get_vcloud_headers(),
                                            verify=self.verify)
                        if response.status_code == requests.codes.ok:
                            media = mediaType.parseString(response.content, True)
                            link = filter(lambda link: link.get_rel() == 'task:cancel', media.get_Tasks().get_Task()[0].get_Link())[0]
                            return link


    def upload_media(self, catalog_name, item_name, media_file_name, description='', display_progress=False, chunk_bytes= 8* 640 *1024):
        """
        ...
        **service type:** ondemand, subscription, vcd
        ...
        for catalog in self.get_catalogs():
            if catalog_name != catalog.name:
                continue
            link = filter(lambda link: link.get_type() == "application/vnd.vmware.vcloud.media+xml" \
                                       and link.get_rel() == 'add', catalog.get_Link())
            assert len(link) == 1
            ...
            headers["Content-Type"] = content_type
            response = self._invoke_api(requests, 'post', link[0].get_href(), data=data, headers=headers,
                                        verify=self.verify)
            if response.status_code == requests.codes.forbidden:
                raise exceptions.ForbiddenException("Upload media iso forbidden %s" % item_name)
            if response.status_code == requests.codes.created:
                catalogItem = ET.fromstring(response.content)
                entity = [child for child in catalogItem if child.get("type") == "application/vnd.vmware.vcloud.media+xml"][0]
                href = entity.get('href')
                response = self._invoke_api(requests, 'get', href, headers=self.vcloud_session.get_vcloud_headers(),
                                            verify=self.verify)
                if response.status_code == requests.codes.forbidden:
                    raise exceptions.ForbiddenException("Get media iso forbidden %s" % item_name)
                if response.status_code == requests.codes.ok:
                    media = mediaType.parseString(response.content, True)
                    ...
                    while bytes_transferred < statinfo.st_size:
                        ...
                        if len(my_bytes) <= chunk_bytes:
                            ...
                            response = self._invoke_api(requests, 'put', link.get_href(), data=my_bytes,
                                                        headers=headers, verify=self.verify)
                            if response.status_code == requests.codes.forbidden:
                                raise exceptions.ForbiddenException("Put media iso forbidden %s" % item_name)
                            if response.status_code == requests.codes.ok:
                                ...
                                Log.debug(self.logger, 'file upload failed with error: [%s] %s' % (response.status_code, response.content))
                                return False
                    f.close()
                    if display_progress:
                        progress_bar.finish()

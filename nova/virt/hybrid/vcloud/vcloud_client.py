...
class VCloudClient(object):
    ...
    def upload_metadata_iso_directly(self, iso_file, vapp_name):
        media_name = "metadata_%s.iso" % vapp_name
        try:
            execute_successful = self._invoke_api("upload_media", self._metadata_iso_catalog, media_name, iso_file)
            time.sleep(5)
        except Exception as e:
            execute_successful = False
            LOG.error('upload media failed without overwrite %s.' % (e))
        if not execute_successful:
            task_link_href = self._invoke_api("get_upload_media_task", self._metadata_iso_catalog, media_name)
            if task_link_href:
                self._invoke_api("cancel_upload", task_link_href)
                time.sleep(5)
                self.delete_metadata_iso(vapp_name)
            raise exception.NovaException(
                "Unable to upload media iso file %s" % vapp_name)
        return self._invoke_api("get_media", self._metadata_iso_catalog, media_name)
